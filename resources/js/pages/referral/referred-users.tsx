...
export default function ReferredUsers() {
  ...
  const { props } = usePage();
  const { referredUsers, userType } = props as ReferredUsersProps;
  ...
  const getTotalCommission = (user: ReferredUser) => {
    return user.referrals?.reduce((total, referral) => total + referral.amount, 0) || 0;
  };
      ...
      <div className="space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-bold">{window.appSettings.formatCurrency(referredUsers.data.reduce((total, user) => total + getTotalCommission(user), 0).toFixed(2))}</h3>
              </div>
                          ...
                          {user.plan ? (
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                {window.appSettings.formatCurrency(user.plan.price)}/{t("month")}
                              </p>
                            </div>
                          ) : (
                        ...
                        {getTotalCommission(user) > 0 && (
                          <div className="text-right">
                            <div className="text-sm font-medium text-green-600">
                              +{window.appSettings.formatCurrency(getTotalCommission(user))}
                            </div>
                          ...
                          {user.referrals.map((referral) => (
                            <div key={referral.id} className="flex justify-between text-sm">
                              ...
                              <span className="font-medium text-green-600">
                                +{window.appSettings.formatCurrency(referral.amount)}
                              </span>
                            </div>
                          ))}
                        </div>
        ...
        {referredUsers.last_page > 1 && (
          <div className="flex justify-center space-x-2">
            {Array.from({ length: referredUsers.last_page }, (_, i) => i + 1).map((page) => (
              <Link
                key={page}
                href={`/referral/referred-users?page=${page}`}
                className={`px-3 py-2 rounded-md text-sm ${
                  page === referredUsers.current_page

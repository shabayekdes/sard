...
class PayTabsPaymentController extends Controller
{
    ...
    public function createInvoicePayment(Request $request)
    {
        try {
            ...
            $cartId = 'invoice_' . $invoice->id . '_' . time();

            // Force PayTabs configuration exactly like plan method
            config([
                'paytabs.profile_id' => $settings['profile_id'],
                'paytabs.server_key' => $settings['server_key'],
                'paytabs.region' => $settings['region'],
            ...
            $pay = paypage::sendPaymentCode('all')
                ...
                ->sendURLs(
                    route('paytabs.invoice.success') . '?cart_id=' . $cartId . '&invoice_token=' . $request->invoice_token,
    ...
    public function invoiceSuccess(Request $request)
    {
        try {
            ...
            if ($cartId && $invoiceToken) {
                // Extract invoice ID from cart_id format: invoice_{id}_{timestamp}
                ...
                if (count($parts) >= 2 && $parts[0] === 'invoice') {
                    ...
                    if ($invoice && $invoice->payment_token === $invoiceToken) {
                        $invoice->createPaymentRecord($invoice->total_amount, 'paytabs', $cartId);
    ...
    public function invoiceCallback(Request $request)
    {
        try {
            ...
            if ($cartId && $respStatus === 'A') {
                // Extract invoice ID from cart_id
                $parts = explode('_', $cartId);
                if (count($parts) >= 2 && $parts[0] === 'invoice') {
                    $invoiceId = $parts[1];
                    $invoice = Invoice::find($invoiceId);
                    ...
                    if ($invoice) {
                        $invoice->createPaymentRecord($invoice->total_amount, 'paytabs', $tranRef ?? $cartId);
                    }
                }
            }

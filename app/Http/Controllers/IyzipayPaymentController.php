...
class IyzipayPaymentController extends Controller
{
    ...
    public function createInvoicePayment(Request $request)
    {
        ...
        try {
            ...
            if (empty($paymentSettings['iyzipay_public_key']) || $paymentSettings['is_iyzipay_enabled'] !== '1') {
                return response()->json(['error' => 'Iyzipay payment not configured'], 400);
            }
            ...
            $checkoutRequest->setCallbackUrl(route('iyzipay.invoice.callback', $validated['invoice_token']));
            $checkoutRequest->setEnabledInstallments([1]);

            $buyer = new Buyer();
            $buyer->setId($invoice->client->id ?? 1);
            $buyer->setName($invoice->client->name ?? 'Customer');
            $buyer->setSurname('User');
            $buyer->setGsmNumber('+1234567890');
            ...
            $shippingAddress = new Address();
            $shippingAddress->setContactName($invoice->client->name ?? 'Customer');
            $shippingAddress->setCity('New York');
            $shippingAddress->setCountry('United States');
            $shippingAddress->setAddress('123 Main Street');
            $shippingAddress->setZipCode('10001');
            ...
            $billingAddress = new Address();
            $billingAddress->setContactName($invoice->client->name ?? 'Customer');
            $billingAddress->setCity('New York');
            $billingAddress->setCountry('United States');
            $billingAddress->setAddress('123 Main Street');
            $billingAddress->setZipCode('10001');
    ...
    public function invoiceCallback(Request $request)
    {
        try {
            ...
            $invoice = Invoice::where('payment_token', $request->route('token'))->first();
            if (!$invoice) {
            ...
            $paymentResult = $this->retrieveIyzipayPayment($token, $paymentSettings);
